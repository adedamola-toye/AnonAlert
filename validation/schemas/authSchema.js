...
export const registerSchema = Joi.object({
  ...
  categoriesHandled: Joi.array()
    .items(
      Joi.string()
        .valid(...ACCEPTABLE_CATEGORIES)
        .insensitive()
    )
    .min(1)
    .required()
    .messages({
      "array.base": "Categories handled must be an array of valid categories",
      "array.min": "At least one category must be provided",
      "any.required": "Categories handled is required",
      "any.only": "Invalid category found in categories handled",
    }),

  statesCovered: Joi.array()
    .items(Joi.string().valid(...ACCEPTABLE_STATES).insensitive())
    .min(1)
    .required()
    .messages({
      "array.base": "States covered must be an array of valid states",
      "array.min": "At least one state must be provided",
      "any.required": "States covered is required",
      "any.only": "Invalid state found in states covered",
    }),
